...
(function ($) {
    ...
    $.cleditor = {
        ...
        defaultOptions: {
            ...
            height: '392', // height not including margins, borders or padding
            controls:     // controls to add to the toolbar
                          "italic strikethrough | size " +
                          "| color removeformat | alignleft center alignright justify | undo redo | " +
    ...
    cleditor = function (area, options) {
        ...
        //Bind the richtext-editor paste event
        $('#'+RICH_TEXT_CONTENT, $(editor.$frame[0].contentWindow.document)).bind('paste', function (e) {
            ...
            execCommand(editor, "inserthtml", pastedText, false, undefined)
            return false
        });

        function updateNGUITextArea(html) {
            ...
            while ((result = patt.exec(html)) != null) {
                ...
                if (index > lastIndex) {
                    htmlArray.push(html.substr(lastIndex, index-lastIndex));
                }
                htmlArray.push(result[0].toLowerCase());
                lastIndex = index + result[0].length;
            }
            if (htmlArray.length > 0) {
                html = htmlArray.join("");
            }
            html = html.replace(new RegExp('<span.*?>(.*?)</span>', 'g'), "$1")
            // console.log(html);
            return html;
        }

        options.updateTextArea = updateNGUITextArea;
        $.fn.SetEditorContent = function (text) {
            focus(editor);
            setTimeout(function () {
                execCommand(editor, "selectAll", text, false, undefined);
                execCommand(editor, "inserthtml", text, false, undefined);
            }, 0);
        }
        
        $.fn.GetEditorContent = function (text) {
            return editor.$area.val();
        }
        
        editor.$main.css("background-color", options.bgColor);
    };
    ...
    function execCommand(editor, command, value, useCSS, button) {
        ...
        if (!ie) {
            console.log(editor, command, value, useCSS, button);
    ...
    function imageUrl(filename) {
        cosole.log("url(" + imagesPath() + filename + ")")
    ...
    function refresh(editor) {
        ...
        doc.write(
            ...
            '<body style="' + options.bodyStyle + ' " ' + 'id="' + RICH_TEXT_CONTENT +  '"></body></html>'
        );
        doc.close();
        ...
        $doc.click(hidePopups)
            .keydown(function (e) {
                ...
                if (ie && getSelection(editor).type == "Control" && e.keyCode == 8) {
                    ...
                    e.preventDefault();
                    execCommand(editor, "inserthtml", '&nbsp&nbsp&nbsp&nbsp', false, undefined)
                    updateTextArea(editor, true);
                }
                else if (e.keyCode == 32) {
                    e.preventDefault();
                    execCommand(editor, "inserthtml", '&nbsp', false, undefined)
                    updateTextArea(editor, true);
                }
